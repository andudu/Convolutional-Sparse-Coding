function [Y,U, ystar, optinf] = cbpdnL_lasso_sn(D, S, L, lambda,lambda_bar, mu, opt)
...
%                   L.M, L.SnWeight, L.ind1, L.ind2 (indx is the rect
...
ystar = zeros(length(L),size(D,3)); % array for storing sink node values
for i = 1:length(L)
    o{i}.MaxMainIter = 10;
    o{i}.verbose = 0;      
    o{i}.Y = [];
    ...
    o{i}.StarWeight = lambda_bar/lambda;
end
...
while k <= opt.MaxMainIter & (r > epri | s > edua),
  ...
  for i = 1:length(L)
      ...
      [temp,o{i}.el,foo] = lasso_fista_sn(Ltemp.M,reshape(permute(a(I1,I2,:),[2,1,3]),length(I1)*length(I2) ...
          ,size(a,3)),lambda,mu,rho,o{i}); % warm starting
      JLp = foo +JLp;
      o{i}.Y = temp;
      temp1 = temp(1:1:end-1,:);
      temp2 = temp(end,:);
      temp1 = reshape(temp1,length(I1),length(I2),size(a,3));
      Y(I1,I2,:) = permute(temp1,[2,1,3]);
      ystar(i,:) = temp2;
      clear a temp1 temp2;
  end
  ...
  if opt.AuxVarObj,
    ...
    Jl1 = sum(abs(vec(bsxfun(@times, opt.L1Weight, X)))) + lambda_bar * sum(abs(vec(ystar)));
  end
