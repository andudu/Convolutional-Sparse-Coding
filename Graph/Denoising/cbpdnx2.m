function [Y,U, optinf] = cbpdnx2(D, S, mu, lambda, opt)
...
while k <= opt.MaxMainIter & (r > epri | s > edua),
  ...
  % Solve X subproblem
  Xf = solvedbi_sm(Df, (rho+mu), DSf + rho*fft2(Y - U), C);
  ...
  if opt.Verbose,
    if opt.AutoRho,
      disp(sprintf(sfms, k, Jfn, Jdf, nX^2, Jl1, r, s, rho));
    else
      disp(sprintf(sfms, k, Jfn, Jdf, nX^2, Jl1, r, s));
    end
  end
  ...
  if opt.AutoRho,
    if k ~= 1 && mod(k, opt.AutoRhoPeriod) == 0,
      ...
      if opt.HighMemSolve && rsf ~= 1,
        C = bsxfun(@rdivide, Df, sum(Df.*conj(Df), 3) + rho + mu);
      end
    end
  end
