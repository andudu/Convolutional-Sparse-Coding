function [Y, S_low, optinf] = cLbpdnlc(D, S, L, lambda, mu1,mu2, opt)

% cbpdnlc -- Convolutional Basis Pursuit DeNoising with Lowpass Component
%
%         argmin_{x_k} (1/2)||\sum_k d_k * x_k + S_low - s||_2^2 +
%                           lambda \sum_k ||x_k||_1 +
%                           (mu1/2) \sum_k ||G_r S_low||_2^2 +
%                           (mu1/2) \sum_k ||G_c S_low||_2^2 + 
%                           \sum_k (mu2/2) <x_k ,LX_k>
%
...
%
% Usage:
%       [Y, S_low, optinf] = cbpdnlc(D, S, lambda, mu, opt);
%
% Input:
%       D           Dictionary filter set (3D array)
%       S           Input image
%       L           Graph Laplacian
%       mu2         Regularization parameter for Graph Laplacian
%       lambda      Regularization parameter (l1)
%       mu1          Regularization parameter (l2 of gradient)
%       opt         Algorithm parameters structure
%
% Output:
%       Y           Dictionary coefficient map set (3D array)
%       S_low           Lowpass component
...
while k <= opt.MaxMainIter & (r > epri | s > edua),
  ...
  % Solve S_low subproblem
  SDX = squeeze(S - DX);
  for l = 1:2,
    S_low = l2gsstep(S_low, SDX, Cgs, mu1);
  end
  clear SDX;
  if ndims(S) > 3,
    S_low = reshape(S_low, [size(S,1) size(S,2) 1 size(S,4)]);
  end
  SZf = Sf - fft2(S_low);
  S_low = squeeze(S_low);
  ...
  % Solve Y subproblem %Laplacian Version
  for i = 1:length(L)
      Ltemp = L{i};
      a = Xr+U;
      I1 = Ltemp.ind1(1):Ltemp.ind2(1);
      ...
      [temp,o{i}.el] = lasso_fista(Ltemp.M,reshape(permute(a(I1,I2,:),[2,1,3]),length(I1)*length(I2) ...
          ,size(a,3)),lambda,mu2,rho,o{i}); % warm starting
      o{i}.Y = temp;
  ...
  clear DXf;
  Jl2g = norm(vec(sqrt(Dv(S_low).^2 + Dh(S_low).^2)))/2.0;
  Jfn = Jdf + lambda*Jl1 + mu1*Jl2g;
