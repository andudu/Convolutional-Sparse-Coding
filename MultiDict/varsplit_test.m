...
[D, X,X_bar, optinf] = varsplit_multilearn(D0, Sh, Sh1, lambda, opt);

tag = '5im32dict512cold';
% Display learned dictionary
o1.grey =1;
o1.unifscale =0;
square_plot(D,o1);
saveas(gcf,['VarsplitResults/',tag,'_dict'],'fig');
o1.grey =0;
a = reshape(sum(abs(X),3),size(X,1),size(X,2),size(X,4));
square_plot(a,o1);
saveas(gcf,['VarsplitResults/',tag,'_highres'],'fig');
a = reshape(sum(abs(X_bar),3),size(X_bar,1),size(X_bar,2),size(X_bar,4));
square_plot(a,o1);
saveas(gcf,['VarsplitResults/',tag,'_lowres'],'fig');
