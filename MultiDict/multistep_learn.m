function [D, Y, optinf] = multistep_learn(D0, S, lambda, opt)
...
%       stpsz       1xN array of image/coefficient size
%
% Output:
...
%cropping matrix
Z_crop = cell(1,size(S,3));
for i = 1:length(opt.stpsz)
   if (opt.stpsz == 1) 
       Z_crop{i} = 1;
   else
       temp = zeros(opt.stpsz(i)); 
       temp(1,1) = 1;
       Z_crop{i} = kron(ones(size(S,1),size(S,2)),temp);
       reshape(Z_crop{i},size(Z_crop{i},1),size(Z_crop{i},2),1,1);
   end
end
...
while k <= opt.MaxMainIter & (rx > eprix|sx > eduax|rd > eprid|sd >eduad),
  ...
  for i = 1:size(Y,3)
      if  Z_crop{i} ~= 1,
        Y(:,:,i,:) = bsxfun(@times,Y(:,:,i,1),Z_crop{i});
      end
  end
