...
%do an averaging
X_zfill = zeros(size(X_d));
for i = 1:4
    disp(['iter: ',num2str(i)]);
    z_ind = randi(256,[2,ceil(.1*256*256)]);
    [temp, ~] = cbpdn_zfill(D, sh, lambda,z_ind, opt);
    X_zfill = X_zfill + temp;
    clear temp;
end
X_zfill = X_zfill / 3;
sh_zfill = scnv(D,X_zfill);
s_zfill = sh_zfill + sl;
ps_nr_zfill = psnr(s_zfill,s_ref);
figure;
imagesc(s_zfill); colormap(gray);
title(['psnr of Average = ',num2str(ps_nr_zfill)]);
