function [Y,U, optinf] = cbpdn_zfill(D, S, lambda,z_ind, opt)
...
%       z_ind       indices to force 0 in the spatial domain.
%                   2xN [(i_1;j_1),\dots,(i_n;j_n)]
%
% Output:
...
zero_crop = ones(size(S,1),size(S,2));
for k = 1:size(z_ind,2)
    zero_crop(z_ind(1,k),z_ind(2,k)) = 0;
end
zero_crop = repmat(zero_crop,[1,1,size(D,3)]);
% Start timer
tstart = tic;
