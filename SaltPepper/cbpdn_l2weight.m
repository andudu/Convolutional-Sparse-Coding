function [Y,U, optinf] = cbpdn_l2weight(D, S, lambda, opt)
...
%   L2Weight         Weighting array spatial for the fidelity term 
...
hstr = 'Itn   Fnc       DFid      l1        r         s          rz        sz     ';
...
ifdot = @(Df,Xf) ifft2(sum(bsxfun(@times, Df, Xf),3), 'symmetric');


while k <= opt.MaxMainIter & ((r > epri | s > edua)|(rz>epriz|sz>eduaz)),

  % Solve X subproblem
  zvf = fft2(Z-V);
  % S convolved with all filters in DFT domain
  dzvf = DHop(zvf);
  Xf = solvedbi_sm(Df, rho, dzvf + rho*fft2(Y - U), C);
  ...
  DconvXr = ifdot(Df,fft2(X));
  if opt.RelaxParam_z ~= 1,
    DconvXr = opt.RelaxParam_z*DconvXr + (1-opt.RelaxParam_z)*Z;
  end
  % Solve Z subproblem
  temp1 = 2*F.*S+sigma*(DconvXr+V);
  temp2 = 2*(F+sigma/2);
  Z = temp1./temp2;
  clear temp1 temp2;
  ...
  if opt.AuxVarObj,
    ...
    Jdf = sum(vec(F).*vec(abs(sum(bsxfun(@times,Df,Yf),3)-Sf).^2))/(2*xsz(1)*xsz(2));
    Jl1 = sum(abs(vec(bsxfun(@times, opt.L1Weight, Y))));
  else
    Jdf = sum(vec(F).*vec((DconvXr-S).^2))/(2*xsz(1)*xsz(2));
    Jl1 = sum(abs(vec(bsxfun(@times, opt.L1Weight, X))));
  end
  Jfn = Jdf + lambda*Jl1;

  nX = norm(X(:)); nY = norm(Y(:)); nU = norm(U(:));
  nDx = norm(DconvXr(:)); nZ = norm(Z(:)); nV = norm(V(:));
  if opt.StdResiduals,
    ...
    rz = norm(vec(DconvXr - Z));
    sz = norm(vec(sigma*(Zprv - Z)));
    epriz = sqrt(Ndx)*opt.AbsStopTol+max(nDx,nZ)*opt.RelStopTol;
    eduaz = sqrt(Ndx)*opt.AbsStopTol+sigma*nV*opt.RelStopTol;    
    ...
    epri = sqrt(Nx)*opt.AbsStopTol/max(nX,nY)+opt.RelStopTol;
    edua = sqrt(Nx)*opt.AbsStopTol/(rho*nU)+opt.RelStopTol;
    rz = norm(vec(DconvXr - Z))/max(nDx,nZ);
    sz = norm(vec(sigma*(Zprv - Z)))/nV;
    epriz = sqrt(Ndx)*opt.AbsStopTol/max(nDx,nZ)+opt.RelStopTol;
    eduaz = sqrt(Ndx)*opt.AbsStopTol/(sigma*nV)+opt.RelStopTol; 
  end
  ...
  if opt.Verbose,
    if opt.AutoRho,
      disp(sprintf(sfms, k, Jfn, Jdf, Jl1, r, s, rz, sz, rho, sigma));
    else
      disp(sprintf(sfms, k, Jfn, Jdf, Jl1, r, s, rz, sz));
    end
  end
...
function opt = defaultopts(opt)
  ...
  if ~isfield(opt,'RelaxParam_z'),
      opt.RelaxParam_z = 1.4;
  end  
