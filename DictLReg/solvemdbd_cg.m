function [x, cgst] = solvemdbd_cg(ah, J, b, tol, mit, isn)
...
Jop = @(u) vec(bsxfun(@times,J,reshape(u,asz)));

wrn = warning('query','MATLAB:ignoreImagPart');
warning('off', 'MATLAB:ignoreImagPart');
[xv,flg,rlr,pit] = pcg(@(u) AhAvop(u)+Jop(u), b(:), tol, mit, [], [], isn);
