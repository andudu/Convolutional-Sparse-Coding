function [G, Y, optinf] = bpdndliu(D0, S, lambda, opt)

% bpdndliu -- BPDN Dictionary Learning (Interleaved Update)
%
...
%
% Usage:
%       [D, X, optinf] = bpdndliu(D0, S, lambda, opt)
%
% Input:
...
while k <= opt.MaxMainIter & (rx > eprix|sx > eduax|rd > eprid|sd >eduad),
  ...
  D = linsolveD(X, sigma, luLd, luUd, SY + sigma*(G - H));
