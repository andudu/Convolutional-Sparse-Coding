function [D, Y, Z, optinf] = cbpdnlcdliu(D0, S, lambda, mu, opt)

% cbpdnlcdliu -- Convolutional BPDN with Lowpass Component Dictionary 
%              Learning (Interleaved Update)
%
...
%
% Usage:
%       [D, Y, Z, optinf] = cbpdnlcdliu(D0, S, lambda, mu, opt)
%
% Input:
...
hstr = ['Itn   Fnc       DFid      l1(x)     l2G(z)    Cnstr     '...
        'r(X)      s(X)      r(D)      s(D) '];
...
while k <= opt.MaxMainIter & (rx > eprix|sx > eduax|rd > eprid|sd >eduad),
  ...
  Xf = solvedbi_sm(Gf, rho, GSZf + rho*fft2(Y - U));
  ...
  Yf = fft2(Y);
  YSZf = sum(bsxfun(@times, conj(Yf), SZf), 4);
  ...
  if strcmp(opt.LinSolve, 'SM'),
    Df = solvemdbi_ism(Yf, sigma, YSZf + sigma*fft2(G - H));
  else
    [Df, cgst] = solvemdbi_cg(Yf, sigma, YSZf + sigma*fft2(G - H), ...
  ...
  Gf = fft2(G);
  GSf = bsxfun(@times, conj(Gf), Sf);
  GYf = sum(bsxfun(@times,Gf,Yf),3);
  GY = ifft2(GYf, 'symmetric');
  clear GYf;
  ...
  Jdf = sum(vec(abs(sum(bsxfun(@times,Gf,Yf),3)-SZf).^2))/(2*xsz(1)*xsz(2));
  clear Yf;
  ...
  optinf.itstat = [optinf.itstat;...
       [k Jfn Jdf Jl1 Jl2g rx sx rd sd eprix eduax eprid eduad rho sigma tk]];
  if opt.Verbose,
    dvc = [k, Jfn, Jdf, Jl1, Jl2g, Jcn, rx, sx, rd, sd];
    if opt.AutoRho,
      dvc = [dvc rho];
    end
    if opt.AutoSigma,
