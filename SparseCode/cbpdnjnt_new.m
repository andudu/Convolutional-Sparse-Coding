function [Y, optinf] = cbpdnjnt_new(D, S, lambda, mu, opt)
...
%                           lambda \sum_k ||x_k||_1 +
%                           mu ||{x_k}||_{2,1}
%                           2 norm in the spatial directions
%
...
%   
% Usage:
%       [Y, optinf] = cbpdnjnt(H, s, lambda, mu, opt)
%
% Input:
%       H           Dictionary filter set (3D array)
%       s           Input image
...
function u = shrink1_scalars(v, a)
  ...
  u = bsxfun(@times,sign(v),max(0, abs(v) - a));
...
function U = shrink2_row_vectors(V, a)
  ...
  n2v = sqrt(sum(sum(V.^2, 1),2));
  n2v(n2v == 0) = 1;
  U = bsxfun(@times, V, bsxfun(@rdivide,max(0, n2v - a),n2v));
...
function U = shrink21(V, a, b, W1)
  ...
  U = shrink2_row_vectors(shrink1_scalars(V,  bsxfun(@times,W1 , a)), b);
