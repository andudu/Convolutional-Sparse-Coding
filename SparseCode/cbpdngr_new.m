function [Y, optinf] = cbpdngr_new(D, S, lambda, mu, opt)

% instead of the gradient of the coefficients, take gradients of the 
% reconstructed image
...
while k <= opt.MaxMainIter & (r > epri | s > edua),
  ...
  if opt.AuxVarObj,
    ...
    Jl1 = lambda*sum(abs(vec(bsxfun(@times, opt.L1Weight,Y))));
