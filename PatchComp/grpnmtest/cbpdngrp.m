function [Y,U, optinf] = cbpdngrp(D, S, lambda, mu, g, opt)
...
%       g           2D Vector containing index values indicating the
%                   group number for each dictionary element. The
%                   first group index is 1 (0 indicates no group).
%                   g_ij = group index of the i,j'th coeeficient
...
%   L21Weight        Weighting array for l21 norm of x
...
%fill in  l1, l21 weights
if size(opt.L1Weight,1) == 1,
    opt.L1Weight = repmat(opt.L1Weight,[size(S,1),1,1]);
end

if size(opt.L1Weight,2) == 1,
    opt.L1Weight = repmat(opt.L1Weight,[1,size(S,2),1]);
end


if size(g,1) == 1;
    g = repmat(g,[size(S,1),1,1]);
end
if size(g,2) == 1;
    g = repmat(g,[1,size(S,2),1]);
end
if size(g,3) == 1;
    g = repmat(g,[1,1,size(D,3)]);
end
...
while k <= opt.MaxMainIter & (r > epri | s > edua),
  ...
  Y = shrink_groups(Xr + U, g, Ng, lambda/rho*opt.L1Weight, mu/rho); %group shrinkage
...
function U = shrink_groups(V, g, Ng, a, b)
  ...
  U = zeros(size(V));
  U(g==0) = shrink1(V(g==0), a(g == 0));
  for l = 1:Ng,  
    U(g==l) = shrink21(V(g == l), a(g == l), b);
  end
