function [D, D_bar, Y, Y_bar, optinf] = superres(D0, D0_bar,S,S_bar, lambda, opt)
...
%
% Output:
%       D           High Res dictionary  size 2n x 2n
%       D_bar       Low Res dictionary   size n x n
...
%   Beta             Parameter for weighing the smaller image
...
if isempty(opt.DictFilterSizes),
  ...
  dsz_bar = [size(D0_bar,1) size(D0_bar,2)];
else
  dsz = opt.DictFilterSizes;
  dsz_bar = opt.DictFilterSizes/2;
end
...
if opt.ZeroMean,
  Pcn_bar = @(x) Pnrm(Pzp_bar(Pzmn(PzpT_bar(x))));
else
  Pcn_bar = @(x) Pnrm(Pzp_bar(PzpT_bar(x)));
end
...
%cropping matrix for Z
Zcrop = zeros(xsz);
for i = 1:xsz(3)
    for j = 1:xsz(4)
        Zcrop(:,:,i,j) = kron(ones(xsz_bar(1:2)),[1,0;0,0]);
    end
end
...
% Main loop
k = 1;
beta = opt.beta; %weigh the lower res fidelity some more
...
while k <= opt.MaxMainIter & ((rx > eprix|sx > eduax|rd > eprid|sd >eduad)|...
  ...
  Xf_bar = solvedbi_sm(Gf_bar, rho_bar/beta, GSf_bar + (rho_bar/beta)*fft2(Y_bar - U_bar));
  ...
  temp = Xr+U; temp2 = Px(temp); 
  Ytemp = shrink((rho*(temp2)+delta*(Y_bar-L))/(rho+delta), (lambda/(rho+delta))*opt.L1Weight);
  Y = PxT(Ytemp); clear temp temp2; %set coefficients to 0;
  Y_bar = shrink((rho_bar*(Xr_bar + U_bar)+delta*(Ytemp+L))/(rho+delta), (lambda/(rho_bar+delta))*opt.L1Weight); %set fixed for now
  ...
  if opt.StdResiduals,
    ...
    ry_delta = norm(vec(Ytemp-Y_bar))/max(nY,nY_bar);
  ...
  if strcmp(opt.LinSolve, 'SM'),
    ...
    Df_bar = solvemdbi_ism(Yf_bar, sigma_bar/beta, YSf_bar + (sigma_bar/beta)*fft2(G_bar - H_bar));
  else
    [Df, ~] = solvemdbi_cg(Yf, sigma, YSf + sigma*fft2(G - H), ...
                              cgt, opt.MaxCGIter, Df(:)); 
    [Df_bar, ~] = solvemdbi_cg(Yf_bar, sigma_bar/beta, YSf_bar + (sigma_bar/beta)*fft2(G_bar - H_bar), ...
                              cgt, opt.MaxCGIter, Df_bar(:));                           
